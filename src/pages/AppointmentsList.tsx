...
import { Clock, Activity, FileText, XCircle, AlertTriangle, Eye, X, Mail, Phone, User } from 'lucide-react';

// --- CONFIGURATION ---
// Standardized VITE_ prefix for environment variables to ensure availability in the browser
...
const AppointmentsList = () => {
  ...
  const isCancellable = (appt) => {
    ...
    // Debugging: Log the parsed date and hours remaining to console
    console.log(`Appointment: ${appt.date} | Parsed: ${apptDate} | Hours Left: ${diffInHours.toFixed(2)}`);
  ...
  const handleCancel = async (appt) => {
    ...
    try {
      ...
      // Trigger Webhook with row-specific patient email
      if (N8N_CANCEL_WEBHOOK) {
        fetch(N8N_CANCEL_WEBHOOK, {
          ...
          body: JSON.stringify({
            ...
            email: appt.email, // Patient email from the database row
    ...
    const fetchUserAppointments = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .order('date', { ascending: true }); // Standard sorting by appointment date
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {appointments.map((appt) => {
              ...
              return (
                <div key={appt.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between gap-5 transition-hover hover:shadow-md">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-1 mt-2 text-sm text-slate-500">
                        <div className="flex items-center gap-2"><Clock size={14} /> {appt.start_time}</div>
                        <div className="flex items-center gap-2"><Mail size={14} /> {appt.email}</div>
                        <div className="flex items-center gap-2"><Phone size={14} /> {appt.phone_number}</div>
                      </div>
          ...
          <div className="bg-white w-full max-w-4xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-gray-50">
              <h3 className="font-semibold text-lg text-gray-800 flex items-center gap-2">
                <FileText size={20} className="text-indigo-600" /> Medical Report Preview
              </h3>
              <button onClick={() => setSelectedReport(null)} className="p-2 hover:bg-gray-200 rounded-full text-gray-500 transition-colors">
                <X size={24} />
              </button>
            </div>
            <div className="flex-grow bg-gray-100">
              <iframe src={selectedReport} className="w-full h-full border-none bg-white" title="Report Preview" />
            </div>
            <div className="bg-white p-3 border-t flex justify-end">
               <a href={selectedReport} download="Medical_Report.pdf" className="text-sm text-indigo-600 hover:underline font-medium">Download File</a>
            </div>
          </div>
        </div>
      )}
